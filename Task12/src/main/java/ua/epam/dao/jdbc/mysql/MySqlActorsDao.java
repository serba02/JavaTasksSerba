...
public class MySqlActorsDao implements ActorsDao {
	...
	private static final String UPDATE = "UPDATE actors SET actors.actor_name=?," +
											"actors.actor_surname=?,actors.actor_experience=? " +
											"WHERE actors.actor_id=?";
	private static final String DELETE = "DELETE from actors  WHERE actors.actor_id=?";
	private static final String INSERT = "INSERT actors (actors.actor_id,actors.actor_name,actors.actor_surname,actors.actor_experience)" +
											"VALUES(?,?,?,?)";
	@Override
	...
	private Actors getActorsFromResultSet(ResultSet resultSet)
		...
		actors.setActorId(resultSet.getInt("actor_id"));
		actors.setActorName(resultSet.getString("actor_name"));
		actors.setActorSurname(resultSet.getString("actor_surname"));
		actors.setActorExperience(resultSet.getInt("actor_experience"));
		return actors;
	}
	...
	public void insert(Actors entity) {
		...
		try(PreparedStatement query = connection
				.prepareStatement(INSERT)){
			query.setInt(1, entity.getActorId());
            query.setString(2, entity.getActorName());
			query.setString(3, entity.getActorSurname());
			query.setInt(4, entity.getActorExperience());
			query.executeUpdate();
		}catch(Exception ex){
			//log
			throw new RuntimeException(ex);
